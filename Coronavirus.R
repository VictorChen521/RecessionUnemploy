...
ui<-fluidPage(
  ...
  fluidRow(br(),br(),br()),
  fluidRow(column(2,selectInput('State','Add a State to compare',choices = c(State$`State and area`),selected = "",multiple = T)),
...
server<-function(input,output,session) {
  ...
  observeEvent(input$map_shape_click$id,
               updateSelectInput(session,'State','Add a State to compare',
                                 choices = c(State$`State and area`),
                                 selected = State[which(State$Code==input$map_shape_click$id),]$`State and area`))
  ...
  StateJobless<-reactive(
    Joblessclaim
  )
  
  IndustryJob<-reactive(
    DT[which(DT$Code==input$map_shape_click$id),]
  )
  
  RecessionFin<-reactive(
    NationalEmployRecess[which(NationalEmployRecess$Period=='Financial'),]
  )
  RecessionCovid<-reactive(
    NationalEmployRecess[which(NationalEmployRecess$Period=='Covid'),]
  )
  ...
  output$StateJobPlot<-renderPlot(
    ggplot(StateJobless()[which(StateJobless()$State %in% input$State),],
  ...
  output$IndustryPlot<-renderPlot(
    grid.arrange(ggplot(IndustryJob()[which(IndustryJob()$Month=='Mar'),],
                 ...
                 ggplot(IndustryJob()[which(IndustryJob()$Month=='Jun'),],
                   ...
                   coord_flip()+xlab(NULL)+theme(legend.position = 'none',plot.title = element_text(hjust = 0.5,face = 'bold'))+scale_fill_manual(values = c('TRUE'='#00BFC4','FALSE'='#F8766D')),
                 nrow=1, top = grid::textGrob(paste(State[which(State$Code==input$map_shape_click$id),]$`State and area`,
